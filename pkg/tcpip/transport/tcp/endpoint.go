...
func (e *endpoint) startMainLoop(handshake bool) *tcpip.Error {
	...
	handshakeFunc := func(handshake, mainLoopInline bool) *tcpip.Error {
		if handshake {
		...
		if mainLoopInline {
			return e.protocolMainLoop(handshake, nil)
		}
		go e.protocolMainLoop(handshake, nil)
		return nil

	}
	if e.route.IsResolutionRequired() {
		go handshakeFunc(handshake, true /* mainLoopInline */) // S/R-SAFE: will be drained before save.
		return nil
	}
	return handshakeFunc(handshake, false /*mainLoopInline */) // S/R-SAFE: will be drained before save.
}
